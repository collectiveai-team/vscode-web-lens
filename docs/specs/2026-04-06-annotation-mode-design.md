...
| `src/adapters/contextFiles.ts` | Persist optional annotation prompt text in the saved context payload |
| Extension host adapter(s) | Handle `annotate:sendToChat` — attach composite image to chat |
| `webview/main.css` | Switch webview shell to a flex column layout so banner + strip do not clip the iframe |
...
| `text` | Temporary HTML `<input>` overlay, committed as SVG `<text>` | Click to place, type to fill, click-away or Enter to commit |
...
Extends the existing `ToolbarState` and instruction-banner pattern. The webview shell should remain flow-based: toolbar, banner, and annotation strip stack vertically above `#browser-frame`, while `#browser-frame` grows to fill the remaining height.
...
5. Build a `ContextBundle` with `screenshot` set to the composited PNG and `annotation` set to the optional prompt text
6. Persist both through the existing context-file pipeline so every backend receives the same image + prompt pair
7. Call `overlay.clear()`
8. Set `annotateActive = false`, update UI
...
The prompt text is carried separately from the image in `ContextBundle.annotation` and written into the saved context text file, so adapters that consume `@` file references preserve both parts of the annotation.
